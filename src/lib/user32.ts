...
export const enumWindowsProc = ffi.Callback(
  ...
  (hWnd: M.HWND, lParam: M.LPARAM): boolean => { // stop loop if return false
...
export function show_hide_one(hWnd: M.HWND, nCmdShow: U.constants.CmdShow): Promise<void | M.HWND> {
...
export function task_get_hwnds(task: Config.Task): Promise<M.HWND[] | void> {
  let t: NodeJS.Timer
...
function _get_hwnds(task: Config.Task): Promise<M.HWND[] | void> {
...
function get_task_hwnd(task: Config.Task): Promise<M.HWND[]> {
  return new Promise((resolve, reject) => {
    if (!task) {
      resolve([])
      return
...
export function filter_hwnd(arr: M.HWND[], opts: Config.Opts): M.HWND[] {
  opts = { ...Config.filterWinRulesDefaults, ...opts }
  const ids = <Set<M.HWND>> new Set()
 ...
 */
function _filter_hwnd(hWnd: M.HWND, opts: Config.Opts): M.BOOLEAN {
...
function validate_rule_title(hWnd: M.HWND, opts: Config.Opts): M.BOOLEAN {
...
function validate_rule_style(hWnd: M.HWND, opts: Config.Opts): M.BOOLEAN {
  const GWL_STYLE = -16
...
function validate_rule_exstyle(hWnd: M.HWND, opts: Config.Opts): M.BOOLEAN {
